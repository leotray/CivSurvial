...
public class TerrainGeneratorMultiplayer : NetworkBehaviour
{
    ...
    private void RequestSeedRpc()
    {
        SendSeedToClient(seed);
    }

    [ObserversRpc]
    private void SendSeedToClient(int receivedSeed)
    {
        seed = receivedSeed;
        InitializeSeed();
        GenerateTerrain();
        ...
        for (float z = 0; z <= depth; z += spawnCheckInterval)
        {
            for (float x = 0; x <= width; x += spawnCheckInterval)
            {
                ...
                foreach (var entry in biome.spawnEntries)
                {
                    ...
                    bool shouldSpawn = ignoreSpawnChance || (prng.NextDouble() <= chance);
