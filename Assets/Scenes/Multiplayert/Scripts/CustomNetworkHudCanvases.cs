...
public class CustomNetworkHudCanvases : MonoBehaviour
{
    #region Types.
    /// <summary>
...
#else   
        // Use PlayerNameHandler (now accessible since we're not in FishNet namespace)
        if (_playerNameInput != null)
        {
            // Set initial name from input field
            PlayerNameHandler.PlayerName = _playerNameInput.text;

            // Connect input field to PlayerNameHandler
            PlayerNameHandler playerNameHandler = GetComponent<PlayerNameHandler>();
            if (playerNameHandler != null)
            {
                _playerNameInput.onEndEdit.AddListener(playerNameHandler.SetPlayerName);
                Debug.Log($"[CustomNetworkHudCanvases] Connected input field to PlayerNameHandler.");
            }
            else
            {
                Debug.LogWarning($"[CustomNetworkHudCanvases] PlayerNameHandler component not found on {gameObject.name}");
            }
        }
